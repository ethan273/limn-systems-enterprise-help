...
- **Reactions** - React with emoji (👍, 👎, ❤️, etc.)
...
- **&gt;=50MB** - Automatically routed to Google Drive (no size limit)
...
- New customer → /Customers/\{Customer Name\} folder
- New order → /Orders/\{Order Number\} folder
...
**Automatic Linking:**
- Production order created → folder created
- Shop drawings uploaded → linked to production order
- QC reports uploaded → linked to production order
...
A: Files &lt;50MB stored in Supabase. Files &gt;=50MB automatically routed to Google Drive with no size limit.
...
A: Files &gt;=50MB automatically uploaded to Google Drive. Folder structure synced. Changes reflected in both systems within 5 minutes.
